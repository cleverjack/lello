SidebarView = Backbone.View.extend({
  ...
  render: function() {
    var editable = player.playlist_collection.where({
      editable: true
    });
    var fixed = player.playlist_collection.where({
      editable: false
    });
    this.setElement(render(this.template, {
      title: 'Playlists',
      search: player.searchText,
      editable: editable,
      fixed: fixed
    }));
  },
  ...
  addPlaylist: function() {
    bootbox.prompt('New Playlist', function(result) {
...
SettingsBarView = Backbone.View.extend({
  ...
  openOptions: function() {
    bootbox.dialog({
      message: render('#control_template', {
