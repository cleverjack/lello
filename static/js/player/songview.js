SongView = Backbone.View.extend({
  ...
  render: function() {
    ...
    if (player.playlist.editable && // playlist is editable
      ...
      if (this.scrollTop) {
        _.defer(function() {
          self.scrollElem.animate({
            scrollTop: tmp_scrolltop + 'px'
          },
          0);
        });
      }
      ...
      bootbox.dialog({
        ...
        buttons: {
          ...
          del: {
            ...
            callback: function() {
              socket.emit('delete_playlist', {
                del: player.playlist._id
              });
    ...
    redrawSong: function(_id) {
      ...
      if (song_tr.length !== 0) {
        ...
        this.$el.find('#' + _id).replaceWith(this.song_template({
          song: player.song_collection.findBy_Id(_id)
        }));
      }
    },
  });
